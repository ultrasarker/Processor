...
pub async fn weather(
    ...
    let url: String = format!("https://api.openweathermap.org/data/2.5/weather?q={location}&appid={key}&units=metric");
    ...
    if json["cod"] == "404" {
        ...
        ctx.send(CreateReply::default().embed(not_found_embed)).await?;
        return Ok(())
    }
    ...
    let weather_embed: CreateEmbed = CreateEmbed::new()
        ...
        .description(format!("{} ({})", json["weather"][0]["main"].to_string().replace("\"", ""), json["weather"][0]["description"].to_string().replace("\"", "")))
        .thumbnail(format!("https://openweathermap.org/img/wn/{}@2x.png", json["weather"][0]["icon"].to_string().replace("\"", "")))
        .fields([
            ("Temp", format!("{}°C", json["main"]["temp"]), true),
            ("Min", format!("{}°C", json["main"]["temp_min"]), true),
            ("Max", format!("{}°C", json["main"]["temp_max"]), true),
            ("Feels Like", format!("{}°C", json["main"]["feels_like"]), true),
    ...
    ctx.send(CreateReply::default().embed(weather_embed)).await?;
    Ok(())
}